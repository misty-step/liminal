...
export async function judgeGuess(puzzle, raw, previous) {
  ...
  if (near) {
    return {
      kind: "judged",
      states: statesFor(puzzle, (id) => (id === near.fails ? "close" : "inside")),
    };
  }
  try {
...
export function resetIfAsked(variant, puzzleId) {
  if (new URLSearchParams(location.search).has("reset")) saveGuesses(variant, puzzleId, []);
}
...
export function shareText(puzzle, guesses, trail, dateLabel, solved) {
  const ids = puzzle.conditions.map((c) => c.id);
  const rows = guesses.map((g, i) =>
    ids.map((id, k) => (k < trail[i].shownAt ? MARK[g.states[id]] : "·")).join(""),
  );
  const score = solved ? `${guesses.length} of ${GUESS_LIMIT}` : `X of ${GUESS_LIMIT}`;
